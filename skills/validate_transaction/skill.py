"""
skills/validate_transaction/skill.py

Skill: Validate Transaction.

Responsabilidad unica de esta Skill: tomar una transaccion ya mapeada
estructuralmente (salida de la Skill normalize_transaction) y:
...
validator.py, en la raiz del proyecto. El unico cambio es de
organizacion: VALID_STATUSES y VALID_CURRENCIES ya no se definen
aqui, sino que se importan desde models/transaction.py, para no
duplicar el modelo con la Skill normalize_transaction.
"""
...
def _convert_date(raw_date):
    """
    Convierte una fecha cruda al formato final YYYY-MM-DD.
    """
    if not raw_date:
        return None
...
def validate_transaction(mapped_transaction: dict) -> ValidationResult:
    ...
    (salida de la Skill normalize_transaction).
    """
